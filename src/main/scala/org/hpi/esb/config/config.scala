...
package object config extends Logging {
  ...
  case class KafkaProducerConfig(bootstrapServers: String, keySerializerClass: String,
                                 valueSerializerClass: String, acks: String, batchSize: Int) extends Logging {
    ...
    def areServerAndSerializerAttributesValid: Boolean =
      checkAttributeHasValue("bootstrap server list", bootstrapServers) &&
        checkAttributeHasValue("key serializer class", keySerializerClass) &&
        checkAttributeHasValue("value serializer class", valueSerializerClass)
